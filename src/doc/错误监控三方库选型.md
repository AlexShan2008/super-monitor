...
> 项目需求： 
1. 能做错误异常报警通知，及类目汇总
2. 能设置当访问超过Ns的时候，为异常，这类也需要报警
3. 如果监控站点，能有一些优化建议最好不过
4. 根据监控数据，我们可以做前端方面的运营报告
...
| `Badjs` | （开源）免费 | 二次开发及运维成本 | 
| `eagle-monitor` | （开源）免费|  二次开发及运维成本 | 
| `Sentry`开源 |（开源）免费| 二次开发及运维成本
...
## eagle-monitor
### 由美团开发的一个开源的前端错误监控`SDK`系统，功能同`Badjs`,但是稳定性及功能全面性不如`Badjs`。
